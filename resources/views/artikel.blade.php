...
<form action="/artikel" method="GET" class="w-full md:w-80 relative">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari artikel..." class="w-full bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-700 text-sm px-4 py-3 rounded-sm outline-none focus:border-kominfo dark:focus:border-kominfo transition-colors text-gray-700 dark:text-gray-200 shadow-sm">
            ...
            @if($artikels->count() > 0)
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
                    @foreach($artikels as $artikel)
                        ...
                        <div class="group border border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800/30 hover:border-kominfo dark:hover:border-kominfo transition-all duration-300 flex flex-col rounded-sm overflow-hidden">
