...
<body class="bg-gray-50 dark:bg-slate-900 text-gray-800 dark:text-gray-200 transition-colors duration-300 font-sans flex flex-col min-h-screen">
    ...
    <div class="flex-grow bg-white dark:bg-slate-900 transition-colors duration-300">
        ...
        <div class="bg-slate-50 dark:bg-slate-800/50 border-b border-gray-200 dark:border-slate-700 py-16 transition-colors duration-300">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row md:items-end justify-between gap-6">
                ...
                <form action="/artikel" method="GET" class="w-full md:w-80 relative">
                    <!-- Simpan parameter kategori jika sedang aktif filter -->
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
            ...
            @if(request()->has('kategori') && request('kategori') != '')
                <div class="mb-8 p-4 bg-blue-50 dark:bg-slate-800/60 border-l-4 border-kominfo dark:border-blue-500 rounded-r-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-sm">
                    <div>
                        <span class="text-sm text-gray-500 dark:text-gray-400">Menampilkan kategori:</span>
                        <span class="text-base font-bold text-kominfo dark:text-blue-400 ml-1">{{ request('kategori') }}</span>
                    </div>
                    <a href="/artikel" class="inline-flex items-center text-xs font-bold text-gray-500 hover:text-red-500 dark:hover:text-red-400 transition-colors">
            ...
            <!-- AREA KONTEN ARTIKEL (Dinamis dari Database) -->
            @if($artikels->count() > 0)
            ...
            @else
                ...
                <div class="text-center py-20">
                    ...
                    <p class="text-gray-500 dark:text-gray-400 mt-2">Coba gunakan kata kunci pencarian yang lain atau hapus filter kategori.</p>
