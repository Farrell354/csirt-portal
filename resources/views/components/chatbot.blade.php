...
<div class="csirt-bot-container fixed bottom-6 right-6 z-[9999]">
    ...
    <!-- Jendela Chat -->
    <div class="csirt-bot-window hidden w-80 sm:w-96 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl rounded-2xl shadow-2xl dark:shadow-[0_0_40px_rgba(0,0,0,0.5)] border border-gray-200/50 dark:border-slate-700/50 overflow-hidden mb-4 transition-all duration-300 origin-bottom-right">
        
        <!-- Header (Secure Terminal Style) -->
        <div class="bg-gradient-to-r from-blue-700 to-cyan-600 p-4 flex justify-between items-center text-white relative overflow-hidden">
            <!-- Glow Ambient -->
            ...
            <!-- Tombol Tutup -->
            <button type="button" onclick="this.closest('.csirt-bot-container').querySelector('.csirt-bot-window').classList.add('hidden')" class="relative z-10 p-1.5 bg-black/10 hover:bg-black/20 rounded-md transition-colors cursor-pointer">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
            </button>
        </div>
        
        <!-- Area Pesan -->
        <div class="csirt-bot-messages csirt-scroll h-80 p-5 overflow-y-auto bg-gray-50/50 dark:bg-slate-950/50 flex flex-col text-sm">
            <!-- Balon Pesan Awal AI -->
        ...
        <!-- Area Input -->
        <div class="p-3 border-t border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex gap-2 items-center">
    ...
    <!-- Tombol Buka Chatbot -->
    <button type="button" onclick="this.closest('.csirt-bot-container').querySelector('.csirt-bot-window').classList.toggle('hidden')" class="w-14 h-14 bg-gradient-to-r from-blue-600 to-cyan-500 hover:from-blue-500 hover:to-cyan-400 text-white rounded-full shadow-[0_0_20px_rgba(59,130,246,0.5)] flex items-center justify-center ml-auto transition-transform hover:scale-110 relative group cursor-pointer pointer-events-auto border-2 border-white/10">
        <!-- Ping Effect Bawaan -->
