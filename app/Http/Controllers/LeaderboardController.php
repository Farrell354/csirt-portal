...
use App\Models\Laporan; // Sesuaikan jika nama model Bos adalah 'Hunter'
use App\Models\User; // Sesuaikan dengan nama model laporan bug-nya
...
class LeaderboardController extends Controller
{
    public function index()
    {
        ...
        $hunters->map(function ($hunter, $index) {
            ...
            if ($hunter->rank == 1) {
                $hunter->pangkat = 'GRAND MASTER';
            } elseif ($hunter->rank == 2) {
                $hunter->pangkat = 'SILVER GUARDIAN';
            } elseif ($hunter->rank == 3) {
                $hunter->pangkat = 'BRONZE STRIKER';
            } elseif ($hunter->total_score >= 100) {
                $hunter->pangkat = 'LVL 3 EXPERT';
            } else {
                $hunter->pangkat = 'LVL 1 ROOKIE';
            }
